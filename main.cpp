...
//  MCLocalisation
//
//  Created by PavanKumar on 18/06/2020.
...
public:
    ...
    double measurement_prob(vector<double> measurement)
    {
        ...
        for (int i = 1; i <= sizeof(landmarks) / sizeof(landmarks[0]); i++) {
            ...
            //cout << typeid(prob).name() << endl;
            //cout << sense_noise << endl;
        }
...
private:
    ...
    double gaussian(double mu, double sigma, double x)
    {
        ...
        double extTerm = exp(-(pow((mu - x), 2)) / ((pow(sigma, 2)) * 2.0));
        double denom = sqrt(2.0 * M_PI * (pow(sigma, 2)));
        //cout<< "weight is : " << extTerm  << endl;
        return extTerm / denom ;
    }
};
...
int main()
{
    ...
    for(int j = 1; j <= steps; j++){
        ...
        for(int i = 1; i<=n; i++){
            beta = beta + gen_real_random() * 2 * max(w, n);
            while (beta > w[index]) {
                beta -= w[index];
                index = mod((index + 1), n);
            }
        ...
        cout << "Step = " << j << ", Evaluation = " << ErrorValue << endl;
